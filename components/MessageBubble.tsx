...
export function MessageBubble({ message, nodeId, onBranchClick }: MessageBubbleProps) {
  const { getBranchingContext } = useBranchContext();

  const renderTextWithBranches = (content: string, branchPoints: BranchPoint[]) => {
    if (branchPoints.length === 0) {
      return content;
    }

    const sortedBranches = [...branchPoints].sort((a, b) => a.startOffset - b.startOffset);
    const segments = [];
    let lastOffset = 0;
      ...
      >
        ...
        <div className="text-xs opacity-70 mt-1">
          ...
          {message.branchPoints.length > 0 && (
            <span className="ml-2">• {message.branchPoints.length} branch{message.branchPoints.length !== 1 ? 'es' : ''}</span>
          )}
        </div>
      </div>
    </motion.div>
